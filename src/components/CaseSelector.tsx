      ...
      <div className="flex flex-col gap-3.5">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-1.5 text-[1.22rem] font-semibold tracking-[0.03em] text-slate-50 sm:text-[1.42rem]">
              可选推理剧本
            </h2>
          ...
          <div className="rounded-[20px] border border-white/8 bg-white/[0.04] px-3.5 py-2.5 text-sm text-[#D6DEEA]">
          ...
          {visibleCases.map((caseDefinition) => {
            ...
            return (
              ...
              >
                ...
                <p className="mt-2.5 text-sm leading-6 text-[#D6DEEA]">{caseDefinition.selectionSummary}</p>
                ...
                <div className="mt-3 flex items-center justify-between text-xs text-[#AEB8C5]">
                  <span>{caseDefinition.npcs.length} 名关键对象</span>
                  <span>{caseDefinition.clues.length} 条核心线索</span>
                </div>

                <div className="mt-2.5 flex items-center justify-between text-xs font-medium tracking-[0.12em] text-[#D7DEE7]">
                  <span>{selected ? "继续当前调查" : "点击进入专属剧本页"}</span>
                  <span aria-hidden="true">→</span>
                </div>

                {selected ? (
                  <div className="mt-3 rounded-[18px] border border-[#8eb2c166] bg-[rgba(142,178,193,0.1)] px-3 py-2 text-xs font-medium text-[#E6F3FA]">
