...
export function CaseFilePanel({
  ...
  return (
    <PanelFrame
      title="Case File"
      subtitle="案件简报、关键证词和证据会固定保存在这里，方便你持续锁定真凶。"
    ...
    >
      <div className="flex h-full min-h-0 flex-col gap-4">
        <section className="cyber-card rounded-[26px] p-4">
          <CaseBriefCard caseFile={caseFile} mode="full" />
        </section>

        <section className="cyber-card rounded-[26px] p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-[0.7rem] uppercase tracking-[0.24em] text-[#AEB8C5]">
                Case Phase
              </p>
              <p className="mt-2 text-lg font-semibold text-[#E2E8F0]">{caseFile.phase}</p>
            </div>
            <div className="rounded-[22px] border border-white/8 bg-white/[0.05] px-3.5 py-2.5 text-right">
              <p className="text-[0.68rem] uppercase tracking-[0.24em] text-[#AEB8C5]">
                Progress
              </p>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            <div className="rounded-[22px] border border-white/8 bg-white/[0.04] px-3.5 py-3">
        ...
        <section className="cyber-card rounded-[26px] p-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-[0.72rem] uppercase tracking-[0.24em] text-[#AEB8C5]">
                Key Testimony
              </p>
            ...
            {testimonyFeed.length ? (
              testimonyFeed.map((testimony) => (
                <article
                  ...
                  className="rounded-[22px] border border-white/8 bg-white/[0.05] p-3.5"
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <span className="terminal-pill rounded-full px-2.5 py-1 text-[0.64rem] uppercase tracking-[0.2em]">
                      {testimony.npcName}
                    </span>
                    <span className="text-[0.68rem] uppercase tracking-[0.2em] text-[#AEB8C5]">
                      {testimony.timestamp}
                    </span>
                  </div>
                  <p className="mt-3 text-[0.92rem] leading-7 text-[#E2E8F0]">
                    {testimony.text}
                  </p>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {testimony.linkedClueIds.map((clueId) => (
                      <span
                        key={clueId}
                        className="rounded-full border border-white/8 bg-white/[0.04] px-2.5 py-1 text-[0.64rem] uppercase tracking-[0.18em] text-[#D7DEE7]"
                      >
              ...
              <div className="rounded-[22px] border border-dashed border-white/8 bg-white/[0.03] px-4 py-5 text-sm leading-7 text-[#AEB8C5]">
        ...
        <section className="cyber-card min-h-0 flex-1 rounded-[26px] p-4">
            ...
            <div className="mt-4 flex-1 space-y-3 overflow-y-auto pr-1">
              {discoveredClues.length ? (
                discoveredClues.map((clue) => (
                  <article
                    key={clue.id}
                    className="rounded-[22px] border border-white/8 bg-[rgba(142,178,193,0.08)] p-4"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-2 text-[0.68rem] uppercase tracking-[0.22em] text-[#AEB8C5]">
                          {clue.category}
                        </p>
                      </div>
                      <span className="rounded-full border border-white/8 bg-white/[0.05] px-2.5 py-1 text-[0.64rem] uppercase tracking-[0.2em] text-[#D7DEE7]">
                        {npcNameById.get(clue.sourceNpcId) ?? clue.sourceNpcId.toUpperCase()}
                      </span>
                    </div>
                    <p className="mt-3 text-[0.92rem] leading-7 text-[#D7DEE7]">
                      {clue.summary}
                    </p>
                  </article>
                ))
