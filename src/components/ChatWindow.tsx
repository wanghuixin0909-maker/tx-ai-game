...
export function ChatWindow({ activeNpc, messages }: ChatWindowProps) {
  ...
  return (
    <PanelFrame title="审问记录" className="chat-panel min-h-0 flex-1 p-4 sm:p-5">
      <div className="cyber-card mb-4 shrink-0 rounded-[24px] px-4 py-3.5 sm:px-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="rounded-[20px] border px-3 py-2" style={{ borderColor: `${activeNpc.accentColor}40`, background: `linear-gradient(180deg, ${activeNpc.accentColor}16, rgba(255,255,255,0.03))` }}>
      ...
      <div ref={messagesContainerRef} className="chat-feed min-h-0 flex-1 overflow-y-auto rounded-[24px] pr-1">
        <div className="space-y-5">
          {messages.map((message) => {
            const isPlayer = message.speakerType === "player";
            const isSystem = message.speakerType === "system";
            return (
              <article key={message.id} className={`flex ${isPlayer ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[88%] rounded-[26px] border px-4 py-3.5 sm:max-w-[78%] ${isPlayer ? "border-white/10 bg-[rgba(167,181,200,0.1)] text-[#E2E8F0] shadow-[0_10px_22px_rgba(9,14,24,0.14)]" : isSystem ? "border-white/8 bg-white/[0.06] text-[#D6DEEA]" : "border-white/8 bg-[rgba(136,145,171,0.08)] text-[#E2E8F0] shadow-[0_10px_22px_rgba(9,14,24,0.12)]"}`}>
                  ...
                  {message.unlockClueIds?.length ? <div className="mt-3 rounded-2xl border border-white/8 bg-white/[0.06] px-3 py-2.5 text-xs leading-5 text-[#D6DEEA]">线索同步: {message.unlockClueIds.length} 条新证据已加入案卷。</div> : null}
                </div>
              </article>
            );
          })}
